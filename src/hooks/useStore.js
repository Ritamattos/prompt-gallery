...
const mapAiCat  = r => ({ id: r.id, name: r.name, icon: r.icon, sortOrder: r.sort_order ?? 0 })
const mapAiTool = r => ({ id: r.id, aiCatId: r.ai_cat_id || null, name: r.name, description: r.description || '', url: r.url || '', img: r.img, sortOrder: r.sort_order ?? 0 })
const EMPTY = { categories: [], subcategories: [], prompts: [], aiTools: [], aiCategories: [] }
...
export function useStore(userId) {
  ...
  async function loadAll() {
    setLoading(true)
    try {
      const [cR, sR, pR, acR, aR] = await Promise.all([
        ...
        supabase.from('ai_categories').select('*').order('sort_order').order('created_at'),
      ...
      const [cFinal, sFinal, pFinal, acFinal, aFinal] = await Promise.all([
        ...
        acR.error ? supabase.from('ai_categories').select('*').order('created_at') : Promise.resolve(acR),
      ...
      if (acFinal.error) console.error('[useStore] ai_categories:', acFinal.error)
      ...
      setData({
        ...
        aiCategories:  (acFinal.data || []).map(mapAiCat),
  ...
  const addAiCategory = useCallback(async (name, icon) => {
    const { data: row, error } = await supabase.from('ai_categories')
      .insert({ user_id: userId, name, icon: icon || '🤖', sort_order: 0 }).select().single()
    throwIf(error, 'addAiCategory')
    setData(d => ({ ...d, aiCategories: [...d.aiCategories, mapAiCat(row)] }))
  }, [userId])

  const updateAiCategory = useCallback(async (id, fields) => {
    const patch = {}
    if (fields.name !== undefined) patch.name = fields.name
    if (fields.icon !== undefined) patch.icon = fields.icon
    const { error } = await supabase.from('ai_categories').update(patch).eq('id', id)
    throwIf(error, 'updateAiCategory')
    setData(d => ({ ...d, aiCategories: d.aiCategories.map(c => c.id === id ? { ...c, ...fields } : c) }))
  }, [])

  const deleteAiCategory = useCallback(async (id) => {
    const { error } = await supabase.from('ai_categories').delete().eq('id', id)
    throwIf(error, 'deleteAiCategory')
    setData(d => ({
      ...d,
      aiCategories: d.aiCategories.filter(c => c.id !== id),
      aiTools:      d.aiTools.map(a => a.aiCatId === id ? { ...a, aiCatId: null } : a),
    }))
  }, [])

  const addAiTool = useCallback(async ({ aiCatId, name, description, url }) => {
    const { data: row, error } = await supabase.from('ai_tools')
      .insert({ user_id: userId, ai_cat_id: aiCatId || null, name, description: description || '', url: url || '', sort_order: 0 })
      .select().single()
  ...
  const updateAiTool = useCallback(async (id, fields) => {
    const patch = {}
    if (fields.aiCatId      !== undefined) patch.ai_cat_id   = fields.aiCatId || null
    if (fields.name         !== undefined) patch.name        = fields.name
  ...
  return {
    ...
    addAiCategory, updateAiCategory, deleteAiCategory,
